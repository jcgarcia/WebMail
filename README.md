...
## Deployment Notes (Ingasti)

### Architecture
- **Platform**: Kubernetes (k3s on oracledev)
- **Image**: `djmaze/snappymail:latest` (v2.38.2)
- **Storage**: PersistentVolumeClaim (local-path, 1Gi)
- **Access**: Protected by Caddy SSO via auth.ingasti.com
- **URL**: https://mail.ingasti.com

### Key Configuration
- **Namespace**: `webmail`
- **Container Port**: 8888 (NOT 80)
- **Mount Point**: `/var/lib/snappymail` (NOT `/var/snappymail/data`)
- **Service Type**: ClusterIP (internal k8s networking only)

### Caddy Integration (oracledev)
```
mail.ingasti.com {
...
**Note**: The service IP (10.43.127.9) changes when namespace is recreated. See TROUBLESHOOTING.md if endless auth loop occurs.

### Authentication Flow
1. User visits https://mail.ingasti.com
2. Caddy checks `authorize with mypolicy` - redirects to auth.ingasti.com if needed
3. User authenticates via GitHub OAuth
4. Caddy passes JWT token back to SnappyMail
5. User sees SnappyMail login page (IMAP credentials)
6. User logs in with their email/password

**Important**: SnappyMail's own login uses IMAP credentials, NOT GitHub SSO. The Caddy SSO only protects access to the domain.

### Known Issues & Fixes
- **Endless auth loop**: See TROUBLESHOOTING.md - usually caused by outdated k8s service IP in Caddyfile
- **Blank page**: Check pod logs: `kubectl -n webmail logs <pod-name>`
- **PVC issues**: Data persists in PVC across restarts - deletion loses configuration
